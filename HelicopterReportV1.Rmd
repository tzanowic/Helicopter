...
screeningtable %>%
  kbl(caption = "Table 2: Reduced Fractional Factorial Model Results") %>%
  kable_classic(full_width = F, html_font = "Cambria") 
```
...
cubePlot(HeliMod_reduced, eff1 = "winglength", eff2 = "paperclips", 
         eff3 = "foldedbody",
         main = "Table 5: Cube Plot for Distance to Optimal Helicopter")
```
...
BBtable %>%
  kbl(caption = "Table 6: Box Behnken Optimization Model Results") %>%
  kable_classic(full_width = F, html_font = "Cambria") 
```
...
persp(model, ~ winglength + paperclips, image = TRUE,
      at = c(summary(model)$canonical$xs, Block="B2"),
      theta=30,zlab="Time",col.lab=33,contour="colors")
mtext("Table 7: Contour Plots for Interactions", side = 3, line = -1.2, outer = TRUE)
```
...
ConfRun %>%
  kbl(caption = "Table 8: Confirmation Runs for Optimal Design") %>%
  kable_classic(full_width = F, html_font = "Cambria") %>%
